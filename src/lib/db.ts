...
import pg, { Pool, type QueryResult, type QueryResultRow } from "pg";
import { attachDatabasePool } from "@vercel/functions";

// The schema uses `bigint` (int8) for ids, transfer_bytes, js_heap_bytes, etc.
// node-postgres returns int8 as a *string* by default, but the committed
// db-types.ts (generated by pg-to-ts) types these columns as `number`. All real
// values here are well within Number.MAX_SAFE_INTEGER, so parse int8 → number to
// keep runtime values consistent with the type contract (and so byte/heap chart
// math works instead of doing string comparisons).
